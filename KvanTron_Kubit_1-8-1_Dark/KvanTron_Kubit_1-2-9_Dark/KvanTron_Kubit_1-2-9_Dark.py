...
# ------------------------------------------------------------
def build_kubbit(base_center=(0,0,0), outer_scale=4.0):
    bx, by, bz = base_center
    traces = []

    # ---- justerat inner_scale: fyra kuber per plan fyller hela ytterkubens dimensioner
    inner_scale = outer_scale / 2.0
...
# 4. SPARA (KANONISKT)
# ------------------------------------------------------------
html_file = "KvanTron_KubBit_1-8-1.html"
fig.write_html(html_file, include_plotlyjs="cdn", auto_open=True)

try:
    fig.write_image("KvanTron_KubBit_1-8-1.png", width=2200, height=2200, scale=2)
except:
    pass
